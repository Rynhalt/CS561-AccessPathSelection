...
Why this is appropriate for your purpose:
- It isolates how clustering affects pruning efficiency.
- Same selectivity probes are run on each layout, so you can directly compare whether speedups come from skipping more work (and, later, from counter movement).
...
Why this is appropriate for your purpose:
- It isolates how key cardinality changes selectivity behavior and pruning opportunities.
- You can compare whether the same predicate class behaves differently when each key is very frequent (low NDV) vs sparse (high NDV).
...
Why this is appropriate for your purpose:
- It isolates skew effects: frequent keys/ranges vs tail keys/ranges.
